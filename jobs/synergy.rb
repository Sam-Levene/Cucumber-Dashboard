...
SCHEDULER.every '2s', :first_in => 0 do |job|
	...
	results1 = connection.query("SELECT COUNT(str.passed) As NumTestsPassed FROM scenario_test_runs str, `time` ti WHERE str.passed  = 1 AND str.updated_at BETWEEN ti.starttime AND ti.endtime")
	results2 = connection.query("SELECT COUNT(str.passed) As NumTestsFailed FROM scenario_test_runs str, `time` ti WHERE str.passed  = 0 AND str.updated_at BETWEEN ti.starttime AND ti.endtime")
